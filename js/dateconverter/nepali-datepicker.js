function getCalendarDivString(t) {
  ...
  return (
    (e += '<span id="ndp-target-id" style="display:none"></span>'),
    ...
    (e += '<span id="currentMonth"></span>'),
    (e += "</div>"),
    (e += '<div id="npd-table-div">'),
    (e += "<table>"),
    (e += '<tr class="ndp-days">'),
    (e += "<th>आ</th>"),
    (e += "<th>सो</th>"),
...
function showNdpCalendarBox(t) {
  if (npdCalendarVisible) hideCalendarBox(!1);
    ...
    var n = $("#" + t).offset();
    $("#ndp-nepali-box").css("top", n.top + $("#" + t).outerHeight()),
      $("#ndp-nepali-box").css("left", n.left),
      showCalendar(e),
...
function showCalendar(t) {
  $("#ndp-nepali-box table").find("tr:gt(0)").remove(),
    "" === t
      ...
      : $("#ndp-nepali-box table").append(getDateTable(t)),
    "block" == $("#ndp-nepali-box").css("display") &&
      $("#ndp-nepali-box").hide(),
...
function getDateTable(t) {
  ...
  if ("" === t) {
    var a = getNepaliDate();
    return (
      (e = getMonthParameters(a)),
      (n = getDateRows(e[0], e[1], e[2], e[3], e[4]))
    );
  }
  return (
    (e = getMonthParameters(t)), (n = getDateRows(e[0], e[1], e[2], e[3], e[4]))
  );
}
function performSelectMonth(t) {
  showCalendar(
    (ndpAttr.npdYear ? $("#npd-year-select").val() : t) +
      "-" +
      $("#npd-month-select").val() +
      "-01"
  );
}
function performSelectYear(t) {
  showCalendar(
    $("#npd-year-select").val() +
      "-" +
      (ndpAttr.npdMonth ? $("#npd-month-select").val() : t) +
      "-01"
  );
}
function getMonthSelect(t, e) {
  var n = getNepaliMonthsInNepali(),
    a =
      '<select id="npd-month-select" onchange="performSelectMonth(' + e + ')">';
  return (
...
function getYearSelect(t, e, n) {
  var a =
    '<select id="npd-year-select" onchange="performSelectYear(' + t + ')">';
  for (
    i =
      Math.round(n / 2) > 0 && parseInt(e) - Math.round(n / 2) >= 1976
        ? parseInt(e) - Math.round(n / 2)
        : 1976;
    i <=
    (Math.round(n / 2) > 0 && parseInt(e) + Math.round(n / 2) <= 2090
      ? parseInt(e) + Math.round(n / 2)
      : 2090);
    i++
  )
...
function getMonthParameters(t) {
  ...
  ndpAttr.npdMonth && ndpAttr.npdYear
    ...
    : ndpAttr.npdYear
    ? $("#currentMonth").html(
        getNepaliMonth(a - 1) + " " + getYearSelect(a, n, s)
      )
    : $("#currentMonth").html(
        getNepaliMonth(a - 1) + "&nbsp;" + englishNo2Nep(n)
      ),
    (nYear = pYear = n),
    (nMonth = parseInt(a, 10) + 1),
    nMonth > 12 && (nYear++, (nMonth = "01")),
    (pMonth = parseInt(a, 10) - 1),
...
function getDateRows(t, e, n, a, s) {
  for (
    ...
    var u = l - t + 1,
      ...
      c = "";
    (c =
      n == d && get2DigitNo(a) == o && h == get2DigitNo(u)
        ? "ndp-selected"
        : u == s
        ? "ndp-current"
        : "ndp-date"),
      ndpAttr.disableDaysBefore &&
        parseInt(ndpAttr.disableDaysBefore) > 0 &&
        (ndpAttr.disableBefore = getNepaliFormat(
          BsAddDays(
            getNepaliFormat(getNepaliDate()),
            -1 * parseInt(ndpAttr.disableDaysBefore)
          )
        )),
      ndpAttr.disableDaysAfter &&
        parseInt(ndpAttr.disableDaysAfter) > 0 &&
        (ndpAttr.disableAfter = getNepaliFormat(
          BsAddDays(
            getNepaliFormat(getNepaliDate()),
            parseInt(ndpAttr.disableDaysAfter)
          )
        )),
      t > l
        ? (p += "<td></td>\n")
        : ((p += "<td class='" + c + "'>"),
          (p +=
            (ndpAttr.disableAfter &&
              BsDatesDiff(ndpAttr.disableAfter, getNepaliFormat(b)) > 0) ||
            (ndpAttr.disableBefore &&
              BsDatesDiff(ndpAttr.disableBefore, getNepaliFormat(b)) < 0)
              ? "<a class=\"ndp-disabled\" href='javascript:void(0)'>" +
                englishNo2Nep(u) +
                "</a>"
...
function hideCalendarBox(t) {
  (t = void 0 !== t ? t : !0),
    ...
    t && ndpAttr.onChange && ndpAttr.onChange();
}
...
function NepaliDateConverter() {
  (this.bs_date_eq = "09/17/1976"),
    (this.ad_date_eq = "01/01/1920"),
    (this.bs = []),
    (this.bs[1976] = [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31]),
    (this.bs[1977] = [30, 32, 31, 32, 31, 31, 29, 30, 29, 30, 29, 31]),
    ...
    (this.bs[1991] = [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 30]),
    ...
    t.fn.nepaliDatePicker = function (e) {
      (e = void 0 !== e ? e : {}),
        ...
        this.each(function () {
          ...
          t(this).addClass("ndp-nepali-calendar"),
            (ndpData[n] = e),
            ndpAttr.onFocus !== !1 &&
              t(this).attr("onfocus", "showNdpCalendarBox('" + n + "')"),
        ...
        t("body").append(getCalendarDivString(ndpAttr)),
        t(".ndp-nepali-calendar, #ndp-nepali-box").hover(
          function () {
            mouse_is_inside = !0;
          },
          function () {
        ...
        t("html").mouseup(function (e) {
          t(e.target).is(".ndp-click-trigger") ||
